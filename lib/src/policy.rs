...
impl ResolutionPolicy for LatestPolicy {
    fn resolve<'a>(&self, name: &str, ontologies: &'a [&'a Ontology]) -> Option<&'a Ontology> {
        ontologies.iter().filter(|o| o.name() == name).max_by_key(|o| o.last_updated).map(|o| *o)
    }
...
impl ResolutionPolicy for VersionPolicy {
    fn resolve<'a>(&self, name: &str, ontologies: &'a [&'a Ontology]) -> Option<&'a Ontology> {
        ...
        let version_vectors: Vec<Vec<String>> = ontologies.iter().filter_map(|o| {
            if o.name() != name {
                return None;
            }
            ONTOLOGY_VERSION_IRIS.iter().map(|iri| {
                let iri: NamedNode = iri.to_owned().into();
                Some(o.version_properties().get(&iri).cloned().unwrap_or_else(|| "0".to_string()))
            }).collect()
        }).collect();
        ...
        let max_index = version_vectors.iter().position(|v| v == max_version).unwrap();
